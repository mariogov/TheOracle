...
use context_graph_core::causal::asymmetric::{infer_direction_from_fingerprint, CausalDirection};
...
impl Handlers {
    ...
    async fn execute_merge(
        ...
        let merged_fingerprint =
            TeleologicalFingerprint::new(merged_semantic, content_hash).with_e6_sparse(e6_sparse);
        ...
        {
            return Err(format!(
                "Failed to store merged content for {}: {}",
                merged_id, e
            ));
        }
        ...
        // PHASE-4: Write permanent MergeRecord to CF_MERGE_HISTORY
        {
            ...
            if let Err(e) = self
                .teleological_store
                .append_merge_record(&merge_record)
                .await
            {
                warn!(
                    merged_id = %merged_id,
    ...
    fn check_fingerprint_compatibility(
        ...
        let has_effect = directions
            .iter()
            .any(|(_, d)| *d == CausalDirection::Effect);
    ...
    fn merge_semantic_union(
        ...
        let e8_graph_as_source = Self::average_dense_vectors(
            fingerprints
                .iter()
                .map(|f| &f.semantic.e8_graph_as_source)
                .collect(),
        );
        let e8_graph_as_target = Self::average_dense_vectors(
            fingerprints
                .iter()
                .map(|f| &f.semantic.e8_graph_as_target)
                .collect(),
        );
        let e9_hdc =
        ...
        SemanticFingerprint {
            ...
            e13_splade,
            // E14: average dense vectors across source fingerprints, skipping
            // legacy/missing (empty) entries. Empty result means no source had
            // E14 populated and the merged fingerprint correctly reports that.
            e14_bge_m3_dense: Self::average_dense_vectors(
                fingerprints
                    .iter()
                    .map(|f| &f.semantic.e14_bge_m3_dense)
                    .filter(|v| !v.is_empty())
                    .collect(),
            ),
        }
    ...
    fn merge_semantic_weighted_average(
        ...
        let (e10_paraphrase_slices, e10_paraphrase_weights): (Vec<&[f32]>, Vec<f32>) = fingerprints
            .iter()
            .zip(weights.iter())
            .filter_map(|(f, &w)| {
        ...
        let (e10_context_slices, e10_context_weights): (Vec<&[f32]>, Vec<f32>) = fingerprints
            .iter()
            .zip(weights.iter())
            .filter_map(|(f, &w)| {
