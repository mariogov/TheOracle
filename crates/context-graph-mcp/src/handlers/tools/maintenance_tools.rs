...
impl Handlers {
    ...
    pub(crate) async fn call_repair_causal_relationships(
        ...
        let Some(rocksdb_store) = store_any
        ...
        match rocksdb_store.repair_corrupted_causal_relationships().await {
            Ok((deleted_count, total_scanned)) => {
                ...
                if let Err(e) = self
                    .append_audit_record_with_readback(&audit_record, "repair_causal_relationships")
                    .await
                {
                    error!(error = %e, "repair_causal_relationships: Audit append/readback failed");
