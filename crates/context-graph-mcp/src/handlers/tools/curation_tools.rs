...
impl Handlers {
    ...
    pub(crate) async fn call_forget_concept(
        ...
        let (request, node_id) = match self.parse_request_validated::<ForgetConceptRequest>(
            id.clone(),
            arguments,
            "forget_concept",
        ) {
        ...
        match self.teleological_store.retrieve(node_id).await {
            ...
            Err(e) => {
                ...
                return self
                    .tool_error(id, &format!("Storage error: Failed to check memory: {}", e));
            }
        };
        ...
        match delete_result {
            Ok(true) => {
                if let Err(e) = self
                    .verify_deleted_fingerprint_readback(node_id, "forget_concept")
                    .await
                {
                    error!(node_id = %node_id, error = %e, "forget_concept: Delete readback failed");
                ...
                if let Err(e) = self
                    .append_audit_record_with_readback(&audit_record, "forget_concept")
                    .await
                {
                    error!(node_id = %node_id, error = %e, "forget_concept: Audit append/readback failed");
                    return self.tool_error(id, &format!("Storage error: {}", e));
                }
                debug!(
                    node_id = %node_id,
                    audit_id = %audit_record.id,
                    "forget_concept: Audit record appended and read back"
                );
    ...
    pub(crate) async fn call_boost_importance(
        ...
        // Parse and validate request
        let (request, node_id) = match self.parse_request_validated::<BoostImportanceRequest>(
            id.clone(),
            arguments,
            "boost_importance",
        ) {
        ...
        match self.teleological_store.update(fingerprint).await {
            Ok(true) => {
                use context_graph_core::types::audit::{
                    AuditOperation, AuditRecord, ImportanceChangeRecord,
                };
                ...
                if let Err(e) = self
                    .append_audit_record_with_readback(&audit_record, "boost_importance")
                    .await
                {
                    error!(node_id = %node_id, error = %e, "boost_importance: Audit append/readback failed");
                    return self.tool_error(id, &format!("Storage error: {}", e));
                }
                debug!(
                    node_id = %node_id,
                    audit_id = %audit_record.id,
                    "boost_importance: Audit record appended and read back"
                );
                ...
                if let Err(e) = self
                    .teleological_store
                    .append_importance_change(&change_record)
                    .await
                {
                    error!(node_id = %node_id, error = %e, "boost_importance: Importance history write failed");
                    return self.tool_error(id, &format!("Storage error: {}", e));
                }
                if let Err(e) = self
                    .verify_importance_readbacks(&change_record, "boost_importance")
                    .await
                {
                    error!(node_id = %node_id, error = %e, "boost_importance: Importance readback failed");
                    return self.tool_error(id, &format!("Storage error: {}", e));
                }
                debug!(
                    node_id = %node_id,
                    old = old_importance,
                    new = new_importance,
                    "boost_importance: Importance update and history row read back"
                );

                // MCP-L1: Use handler's actual computed values, not recomputed from delta
                let response = BoostImportanceResponse::from_result(
                    node_id,
                    old_importance,
                    new_importance,
                    clamped,
