...
//!
//! Per Constitution v6.5 and the 14-embedder philosophy:
...
impl super::validate::Validate for SearchRobustRequest {
...
mod tests {
    ...
    fn test_validation_rejects_invalid() {
        ...
        let bad_k = SearchRobustRequest {
            query: "test query".to_string(),
            top_k: 100,
            ..Default::default()
        };
        assert!(bad_k.validate().is_err());
        let bad_thresh = SearchRobustRequest {
            query: "test query".to_string(),
    ...
    fn test_response_serialization() {
        ...
        let not_discovery = BlindSpotCandidate {
            memory_id: Uuid::new_v4(),
            e9_score: 0.20,
            e1_score: 0.75,
            divergence: -0.55,
