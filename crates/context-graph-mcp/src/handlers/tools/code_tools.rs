...
use super::code_dtos::{
    CodeEntityResult, CodeSearchMetadata, CodeSearchMode, CodeSearchResult, CodeSourceInfo,
    DetectedLanguageInfo, SearchCodeRequest, SearchCodeResponse,
};
...
impl Handlers {
    ...
    /// # Algorithm
    ///
    /// 1. Embed the code query using all 14 embedders
    ...
    /// Search CodeStore for code entities using full 14-embedder fingerprint.
    ///
    ...
    ///
    /// # Constitution Compliance
    /// - ARCH-01: Uses full SemanticFingerprint (all 14 embeddings)
    ...
    async fn search_code_entities(
        ...
        // Generate full fingerprint for query (all 14 embeddings)
...
const LANGUAGE_PATTERNS: &[(&str, &[&str])] = &[
    (
        "rust",
        &[
            "impl ", "fn ", "let mut", "struct ", "enum ", "pub fn", "trait ", "cargo", ".rs",
            "unwrap(", "match ",
        ],
    ),
    (
