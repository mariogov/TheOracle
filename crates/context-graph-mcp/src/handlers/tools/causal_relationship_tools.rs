...
//! of retained causal descriptions with full provenance.
//!
...
#![allow(dead_code)] // Retired E5 relationship search surface; dispatch fails closed.
...
/// Source-anchored embeddings prevent explanation-template clustering.
...
impl Handlers {
    ...
    pub(crate) async fn call_search_causal_relationships(
        ...
        if !context_graph_core::weights::E11_ENTITY_ENABLED && e11_weight > 0.0 {
            return self.tool_error(id, "E11 entity embedder is disabled; e11Weight must be 0.0");
        }
        ...
        if !(0.0..=1.0).contains(&min_consensus) {
            return self.tool_error(
                id,
                &format!(
                    "minConsensus must be between 0.0 and 1.0, got {}",
                    min_consensus
                ),
            );
        }
        ...
        if multi_embedder && matches!(search_mode, "causes" | "effects") {
            ...
            if !context_graph_core::weights::E11_ENTITY_ENABLED && e11_weight > 0.0 {
                return self.tool_error(
                    id,
                    "E11 entity embedder is disabled; multi-embedder causal search cannot use E11",
                );
            }
