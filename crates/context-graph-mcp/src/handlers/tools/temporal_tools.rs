...
impl Handlers {
    ...
    pub(crate) async fn call_search_recent(
        ...
        let query_embedding = match self
            .embed_query(id.clone(), &params.query, "search_recent")
            .await
        {
            Ok(fp) => fp,
    ...
    pub(crate) async fn call_search_periodic(
        ...
        if let Some(hour) = params.target_hour {
            if hour > 23 {
                ...
                return self.tool_error(id, &format!("targetHour must be 0-23, got {}", hour));
            }
        }
        ...
        // Generate query embedding
        let query_embedding = match self
            .embed_query(id.clone(), &params.query, "search_periodic")
            .await
        {
            Ok(fp) => fp,
            Err(resp) => return resp,
        ...
        let mut boosted_results: Vec<PeriodicSearchResultEntry> = results
            ...
            .map(|r| {
                ...
                // TeleologicalFingerprint.semantic has the 14-embedder SemanticFingerprint
