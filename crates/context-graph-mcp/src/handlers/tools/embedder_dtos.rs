...
/// Embedder identifier enum matching the 14 embedders in the system.
...
pub enum EmbedderId {
    ...
    E14, // V_multilingual_style - BGE-M3 dense (XLM-RoBERTa-Large)
}

impl EmbedderId {
    /// Convert from string to EmbedderId.
    ///
    /// Accepts `"E1"`..`"E14"` (case-insensitive) as well as the canonical
    /// `"bge_m3_dense"` name (for compatibility with factory/ModelId wiring).
    pub fn parse(s: &str) -> Option<Self> {
        ...
        match upper.as_str() {
            ...
            "E14" => Some(EmbedderId::E14),
            _ => {
                // Secondary lowercase aliases for the newer E14 slot.
                match s.to_lowercase().as_str() {
                    "bge_m3_dense" | "bge-m3-dense" | "bgem3dense" => Some(EmbedderId::E14),
                    _ => None,
                }
            }
        }
    ...
    /// Get the index in the TeleologicalArray (0-13).
    ...
    pub fn name(&self) -> &'static str {
        match self {
            ...
            EmbedderId::E14 => "V_multilingual_style (BGE-M3 Dense)",
        }
    }
    ...
    pub fn finds(&self) -> &'static str {
        match self {
            ...
            EmbedderId::E14 => "Multilingual semantic/style via BGE-M3 (XLM-RoBERTa)",
        }
    }
    ...
    /// E14 BGE-M3 Dense is HNSW-indexed as a 1024D Cosine embedder.
    ...
    pub fn hnsw_unsupported_error(embedder_name: &str) -> String {
        format!(
            "Embedder '{}' does not support HNSW search. E6/E13 use inverted indexes, E12 uses MaxSim. Use E1-E5, E7-E11, or E14 instead.",
            embedder_name
        )
    }
...
pub struct SearchByEmbedderRequest {
    /// Which embedder to use as primary (E1-E14).
    pub embedder: String,
    /// Search query.
    pub query: String,
    ...
    /// Include scores from all 14 embedders (default: false).
    #[serde(default)]
...
impl SearchByEmbedderRequest {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        match EmbedderId::parse(&self.embedder) {
            None => {
                return Err(format!(
                    "Invalid embedder '{}'. Must be E1-E14.",
                    self.embedder
                ));
            }
            Some(eid) => {
    ...
    pub fn embedder_id(&self) -> Option<EmbedderId> {
        EmbedderId::parse(&self.embedder)
    }
}
...
pub struct EmbedderSearchResult {
    ...
    /// Scores from all 14 embedders (if includeAllScores=true).
    #[serde(skip_serializing_if = "Option::is_none")]
    pub all_scores: Option<AllEmbedderScores>,
}

/// Scores from all 14 embedders for a single memory.
#[derive(Debug, Clone, Serialize)]
pub struct AllEmbedderScores {
    ...
    pub e12: f32,
    pub e13: f32,
    /// E14 BGE-M3 Dense cosine score (1024D, XLM-RoBERTa-Large).
    pub e14: f32,
}
...
pub struct GetEmbedderClustersRequest {
    /// Which embedder's clusters to explore (E1-E14).
    pub embedder: String,
...
impl CompareEmbedderViewsRequest {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        for e in &self.embedders {
            match EmbedderId::parse(e) {
                None => {
                    return Err(format!("Invalid embedder '{}'. Must be E1-E14.", e));
                }
                Some(eid) => {
                    if !eid.uses_hnsw() {
    ...
    pub fn embedder_ids(&self) -> Vec<EmbedderId> {
        self.embedders
            .iter()
            .filter_map(|e| EmbedderId::parse(e))
            .collect()
    }
}
...
pub struct ListEmbedderIndexesResponse {
    /// Information for all 14 embedders.
...
pub struct GetMemoryFingerprintRequest {
    ...
    /// Filter to specific embedders (default: all 14).
    #[serde(default)]
    pub embedders: Vec<String>,
...
impl GetMemoryFingerprintRequest {
    ...
    /// Get the set of requested embedder IDs (empty = all 14).
...
pub struct GetMemoryFingerprintResponse {
    ...
    /// Active embedders with vectors present.
    pub embedders_present: usize,
    /// Number of active content embedders that may contribute to ME-JEPA/search.
    pub active_embedder_count: usize,
    /// Number of persisted storage slots in each TeleologicalFingerprint.
    pub storage_slot_count: usize,
    /// Retired or disabled slots kept only for schema compatibility.
    pub disabled_embedders: Vec<String>,
...
pub struct CreateWeightProfileRequest {
    ...
    pub name: String,
    /// Per-embedder weights as E1-E14 keys. Must sum to ~1.0.
...
impl CreateWeightProfileRequest {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        for key in self.weights.keys() {
            if EmbedderId::parse(key).is_none() {
                return Err(format!(
                    "Invalid embedder name '{}' in weights. Valid names: E1-E14.",
                    key
                ));
            }
        }
    ...
    /// Convert the weights map to a [f32; 14] array.
    pub fn to_weight_array(&self) -> [f32; 14] {
        let mut weights = [0.0f32; 14];
        for (key, &val) in &self.weights {
            if let Some(eid) = EmbedderId::parse(key) {
...
pub struct CreateWeightProfileResponse {
    ...
    pub name: String,
    /// The stored weights as E1-E14 map.
...
pub struct SearchCrossEmbedderAnomaliesRequest {
    /// Search query.
    pub query: String,
    /// Embedder expected to score HIGH (E1-E14).
    pub high_embedder: String,
    /// Embedder expected to score LOW (E1-E14).
...
impl SearchCrossEmbedderAnomaliesRequest {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if EmbedderId::parse(&self.high_embedder).is_none() {
            return Err(format!(
                "Invalid highEmbedder '{}'. Must be E1-E14.",
                self.high_embedder
            ));
        }
        if EmbedderId::parse(&self.low_embedder).is_none() {
            return Err(format!(
                "Invalid lowEmbedder '{}'. Must be E1-E14.",
                self.low_embedder
            ));
        }
...
fn default_expansion_min_score() -> f32 {
    0.1
}
...
fn default_primary_embedder() -> String {
    "E1".to_string()
}
fn default_contrast_embedder() -> String {
    "E9".to_string()
}
...
mod tests {
    ...
    fn test_happy_path_deserialization() {
        assert_eq!(EmbedderId::parse("E1"), Some(EmbedderId::E1));
        assert_eq!(EmbedderId::parse("e11"), Some(EmbedderId::E11));
        // Post-E14: the 14th slot is BGE-M3 Dense. E15 is the first invalid slot.
        assert_eq!(EmbedderId::parse("E14"), Some(EmbedderId::E14));
        assert_eq!(
            EmbedderId::parse("bge_m3_dense"),
            Some(EmbedderId::E14),
            "canonical lowercase alias must resolve to E14"
        );
        assert_eq!(EmbedderId::parse("E15"), None);
        assert_eq!(EmbedderId::E1.to_index(), 0);
        assert_eq!(EmbedderId::E13.to_index(), 12);
        assert_eq!(EmbedderId::E14.to_index(), 13);
        assert!(EmbedderId::E1.is_semantic());
        assert!(EmbedderId::E14.is_semantic());
        assert!(EmbedderId::E2.is_temporal());
        assert!(EmbedderId::E6.is_sparse());
        assert!(EmbedderId::E14.uses_hnsw());
    }
    ...
    fn test_validation_rejects_invalid() {
        ...
        // E14 is valid post-integration; E15 is the first invalid embedder name.
        let valid_e14 = SearchByEmbedderRequest {
            embedder: "E14".to_string(),
            query: "multilingual query".to_string(),
            top_k: 10,
            min_similarity: 0.0,
            include_content: false,
            include_all_scores: false,
        };
        assert!(valid_e14.validate().is_ok());
        let invalid_embedder = SearchByEmbedderRequest {
            embedder: "E15".to_string(),
