...
//! # CUDA 13.2 Note
//!
//! The workspace Candle stack targets CUDA 13.2 / compute capability 12.0.
//! This module still validates GPU-batched behavior explicitly because silent
//! CPU fallback is forbidden for production paths.
...
impl DimensionGroup {
    ...
    pub const fn all() -> [DimensionGroup; 4] {
        [Self::Dim1024, Self::Dim768, Self::Dim512, Self::Dim1536]
    }
...
/// # Note on CUDA 13.2
///
/// CUDA 13.2 / sm_120 support is required for production. Callers must treat
/// any unavailable GPU path as a hard error rather than falling back silently.
...
mod tests {
    ...
    fn test_batch_similarity_identical_vectors() {
        ...
        let query: [&[f32]; 10] = [&e1, &e2, &e3, &e4, &e5, &e7, &e8, &e9, &e10, &e11];

        let query_ctx = BatchedQueryContext::new(&query).unwrap();

        // Candidate with identical vectors
        let candidate: [&[f32]; 10] = [&e1, &e2, &e3, &e4, &e5, &e7, &e8, &e9, &e10, &e11];
    ...
    fn test_batch_similarity_orthogonal_vectors() {
        ...
        let query: [&[f32]; 10] = [&e1_q, &e2, &e3, &e4, &e5, &e7, &e8, &e9, &e10, &e11];

        let query_ctx = BatchedQueryContext::new(&query).unwrap();

        let candidate: [&[f32]; 10] = [&e1_c, &e2, &e3, &e4, &e5, &e7, &e8, &e9, &e10, &e11];
