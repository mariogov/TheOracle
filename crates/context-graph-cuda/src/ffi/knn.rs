...
//! Uses native sm_120a cubin loaded via Driver API to avoid cudart static
//! initialization bugs on WSL2 with CUDA 13.x and to avoid PTX JIT startup cost.
//!
...
// Include generated native cubin image.
include!(concat!(env!("OUT_DIR"), "/knn_image.rs"));
...
pub(crate) fn ensure_cuda_initialized() -> CudaResult<()> {
    INIT.call_once(|| {
...
impl GpuBuffer {
    /// Allocate GPU memory.
    pub(crate) fn new(size: usize) -> CudaResult<Self> {
    ...
    pub(crate) fn copy_from_host(&self, src: &[u8]) -> CudaResult<()> {
    ...
    pub(crate) fn copy_to_host(&self, dst: &mut [u8]) -> CudaResult<()> {
        ...
        let ret = unsafe { cuMemcpyDtoH_v2(dst.as_mut_ptr() as *mut c_void, self.ptr, dst.len()) };
...
impl CudaContext {
    pub(crate) fn new(device: i32) -> CudaResult<Self> {
        let mut dev: c_int = 0;
...
impl CudaModule {
    pub(crate) fn load_image(image: &[u8]) -> CudaResult<Self> {
        if image.is_empty() {
            return Err(CudaError::InvalidArgument {
                argument: "image".to_string(),
                reason: "CUDA cubin image is empty".to_string(),
            });
        }
        let mut module: CUmodule = std::ptr::null_mut();
        let ret = unsafe { cuModuleLoadData(&mut module, image.as_ptr() as *const c_void) };
    ...
    pub(crate) fn get_function(&self, name: &str) -> CudaResult<CUfunction> {
        ...
        let ret = unsafe { cuModuleGetFunction(&mut func, self.module, name_cstring.as_ptr()) };
