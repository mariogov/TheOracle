...
use crate::gpu::{init_gpu, normalize_gpu, GpuModelLoader};
use crate::traits::{EmbeddingModel, SingleModelConfig};
use crate::types::{InputType, ModelEmbedding, ModelId, ModelInput};

use super::super::bert_batch::{gpu_forward_text_batch, BertBatchPooling, BertBatchSpec};
use super::constants::{
    CONTEXTUAL_DIMENSION, CONTEXTUAL_MAX_TOKENS, CONTEXT_PREFIX, INTENT_PREFIX,
};
...
impl ContextualModel {
    ...
    /// True CUDA batch processing using one padded E5-base tensor forward pass.
    pub async fn embed_batch(&self, inputs: &[ModelInput]) -> EmbeddingResult<Vec<ModelEmbedding>> {
        ...
        if inputs.is_empty() {
            return Err(EmbeddingError::TrueBatchEmpty {
                model_id: ModelId::Contextual,
                recovery_hint:
                    "submit at least one ContextualModel input; empty batches are invalid"
                        .to_string(),
            });
        }
        ...
        for input in inputs {
            ...
            prepared.push(format!("{}{}", CONTEXT_PREFIX, content));
        }
        ...
        vectors
            .into_iter()
            .map(|vector| {
                let embedding = ModelEmbedding::new(ModelId::Contextual, vector, latency_us);
                embedding.validate()?;
                Ok(embedding)
            })
            .collect()
    ...
    pub async fn embed_dual(&self, content: &str) -> EmbeddingResult<(Vec<f32>, Vec<f32>)> {
        ...
        match &*state {
            ModelState::Loaded { weights, tokenizer } => {
                ...
                if intent_vec.len() != CONTEXTUAL_DIMENSION
                    || context_vec.len() != CONTEXTUAL_DIMENSION
                {
...
fn self_attention(
    ...
    // Softmax
    let attention_probs =
        candle_nn::ops::softmax(&scores, 3).map_err(|e| EmbeddingError::GpuError {
    ...
    context_flat
        .matmul(
            &att.output_weight
                ...
                .map_err(|e| EmbeddingError::GpuError {
                    message: format!(
                        "ContextualModel attention output weight transpose failed: {}",
                        e
                    ),
                })?,
        )
    ...
    let intermediate = hidden_flat
        .matmul(
            &ffn.intermediate_weight
                ...
                .map_err(|e| EmbeddingError::GpuError {
                    message: format!(
                        "ContextualModel ffn intermediate weight transpose failed: {}",
                        e
                    ),
                })?,
        )
...
impl EmbeddingModel for ContextualModel {
    ...
    async fn unload(&self) -> EmbeddingResult<()> {
        ContextualModel::unload(self).await
    }
    ...
    async fn embed_true_batch(
        ...
        ContextualModel::embed_batch(self, inputs).await
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_embed_true_batch_returns_ordered_768d_vectors() {
        use crate::models::pretrained::shared::pretrained_test_model_path;
        use crate::traits::SingleModelConfig;
        use crate::types::ModelInput;

        let model_path = pretrained_test_model_path("contextual-e5-base");
        let model = ContextualModel::new(&model_path, SingleModelConfig::default())
            .expect("create contextual model");
        model.load().await.expect("load contextual model");
        assert!(model.supports_true_batch());

        let inputs = vec![
            ModelInput::text("intent traces explain why the code failed").expect("input"),
            ModelInput::text("context chunks preserve durable reality evidence").expect("input"),
        ];
        let embeddings = EmbeddingModel::embed_true_batch(&model, &inputs)
            .await
            .expect("contextual true batch");

        assert_eq!(embeddings.len(), inputs.len());
        for embedding in &embeddings {
            embedding
                .validate()
                .expect("valid contextual true-batch vector");
            assert_eq!(embedding.vector.len(), CONTEXTUAL_DIMENSION);
        }
    }
    ...
    fn test_layer_norm_different_batch_sizes() {
        ...
        for batch_size in [1, 2, 4] {
            let hidden_states = Tensor::randn(0.0f32, 1.0, (batch_size, 64, 768), &device).unwrap();
            let weight = Tensor::ones((768,), candle_core::DType::F32, &device).unwrap();
            let bias = Tensor::zeros((768,), candle_core::DType::F32, &device).unwrap();
    ...
    fn test_layer_norm_normalizes_output() {
        ...
        let output_mean = output.mean_keepdim(candle_core::D::Minus1).unwrap();
    ...
    fn test_layer_norm_different_sequence_lengths() {
        ...
        for seq_len in [1, 16, 128, 384] {
            let hidden_states = Tensor::randn(0.0f32, 1.0, (1, seq_len, 768), &device).unwrap();
            let weight = Tensor::ones((768,), candle_core::DType::F32, &device).unwrap();
            let bias = Tensor::zeros((768,), candle_core::DType::F32, &device).unwrap();
