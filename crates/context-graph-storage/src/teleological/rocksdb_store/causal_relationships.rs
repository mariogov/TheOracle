...
/// Audit-14 STOR-L4 FIX: Generic scan-and-score helper for causal relationship search.
///
/// Extracts the shared synchronous boilerplate called from within `spawn_blocking`:
/// 1. Getting CF handle and creating iterator
/// 2. Deserializing each CausalRelationship (with error handling)
/// 3. Calling a user-provided scoring closure
/// 4. Sorting by score descending and truncating to `top_k`
///
/// # Arguments
/// * `db` - Arc-wrapped RocksDB instance
/// * `top_k` - Number of top results to return
/// * `scorer` - Closure that takes a `&CausalRelationship` and returns `Option<(Uuid, f32)>`.
///   Return `None` to skip the relationship (e.g., filter or missing embedding).
fn scan_causal_relationships_scored<F>(
    db: Arc<rocksdb::DB>,
    top_k: usize,
    scorer: F,
) -> CoreResult<Vec<(Uuid, f32)>>
where
    F: Fn(&CausalRelationship) -> Option<(Uuid, f32)> + Send + 'static,
{
    let cf = db
    ...
    for item in iter {
        ...
        if let Some(scored) = scorer(&relationship) {
            results.push(scored);
        }
    }
...
impl RocksDbTeleologicalStore {
    ...
    pub async fn search_causal_relationships(
        ...
        // Audit-14 STOR-L4 FIX: Use scan_causal_relationships_scored helper.
        let results = tokio::task::spawn_blocking(move || {
            scan_causal_relationships_scored(db, top_k, move |rel| {
    ...
    pub async fn search_causal_e5_hybrid(
        ...
        let results = tokio::task::spawn_blocking(move || {
            scan_causal_relationships_scored(db, top_k, move |rel| {
                ...
                let explanation_sim =
                    compute_cosine_similarity(&query_embedding, explanation_embedding);

                let source_sim =
                    if source_embedding.is_empty() || source_embedding.iter().all(|&v| v == 0.0) {
                        explanation_sim
                    } else {
                ...
                let hybrid_score =
                    source_weight * source_sim + explanation_weight * explanation_sim;
                Some((rel.id, hybrid_score))
            })
        })
        .await
        .map_err(|e| CoreError::Internal(format!("spawn_blocking failed: {}", e)))??;
    ...
    pub async fn search_causal_e8(
        ...
        let results = tokio::task::spawn_blocking(move || {
            scan_causal_relationships_scored(db, top_k, move |rel| {
                ...
                if !rel.has_graph_embeddings() {
                    return None;
                }

                // Select appropriate E8 vector based on search mode
                let doc_embedding = if search_sources {
                    rel.e8_graph_source_embedding()
                } else {
                    rel.e8_graph_target_embedding()
                };
    ...
    pub async fn search_causal_e11(
        ...
        let filtered = tokio::task::spawn_blocking(move || -> CoreResult<Vec<(Uuid, f32)>> {
            ...
            let cf = db.cf_handle(CF_CAUSAL_RELATIONSHIPS).ok_or_else(|| {
                CoreError::Internal("CF_CAUSAL_RELATIONSHIPS not found".to_string())
            })?;
            // F-017 fail-closed (Sherlock investigation 2026-05-19):
            // the legacy comprehension used `.ok().flatten().is_some()`,
            // which silently treated RocksDB I/O / corruption errors as
            // "row not found" and dropped the match. The current
            // implementation surfaces those errors via `?` so corruption
            // cannot masquerade as a clean miss.
            let mut filtered: Vec<(Uuid, f32)> = Vec::with_capacity(results.len().min(top_k));
            for (id, score) in results.into_iter() {
                if filtered.len() >= top_k {
                    break;
                }
                let key = causal_relationship_key(&id);
                let row = db.get_cf(cf, key).map_err(|err| {
                    CoreError::Internal(format!(
                        "MEJEPA_CAUSAL_PRIMARY_LOOKUP_FAILED: id={} err={}",
                        id, err
                    ))
                })?;
                if row.is_some() {
    ...
    pub async fn repair_corrupted_causal_relationships(&self) -> CoreResult<(usize, usize)> {
        ...
        let orphan_count = tokio::task::spawn_blocking(move || -> CoreResult<usize> {
            ...
            for item in iter {
                ...
                for causal_id in &causal_ids {
                    ...
                    // F-017 fail-closed (Sherlock investigation 2026-05-19):
                    // the legacy `.ok().flatten().is_some()` swallowed RocksDB
                    // I/O errors and silently classified them as "orphan",
                    // which would then DELETE a still-valid secondary-index
                    // entry. The current implementation surfaces the I/O
                    // error via `?` so the entire repair operation aborts
                    // (preserving the secondary index) rather than corrupting
                    // it further.
                    let primary_row = db2.get_cf(cf_primary, primary_key).map_err(|err| {
                        CoreError::Internal(format!(
                            "MEJEPA_CAUSAL_PRIMARY_LOOKUP_FAILED: causal_id={} err={}",
                            causal_id, err
                        ))
                    })?;
                    if primary_row.is_some() {
        ...
        if orphan_count > 0 {
            info!(
                orphan_count,
                "Removed orphaned secondary index entries from CF_CAUSAL_BY_SOURCE"
            );
        }
        ...
        info!(
            ...
            "Causal relationship repair complete"
        );
        Ok((deleted_count, total_scanned))
    }
    ...
    pub async fn delete_causal_relationship(&self, id: Uuid) -> CoreResult<bool> {
            ...
            Ok(Some(bytes)) => {
                let mut causal_ids: Vec<Uuid> = serde_json::from_slice(&bytes).map_err(|e| {
                    ...
                    TeleologicalStoreError::Internal(format!(
                        "Corrupted causal_by_source index for {}: {}",
                        relationship.source_fingerprint_id, e
                    ))
                })?;
                ...
                if causal_ids.is_empty() {
                    ...
                    let serialized = serde_json::to_vec(&causal_ids).map_err(|e| {
                        CoreError::Internal(format!(
                            "Failed to serialize causal_by_source list: {}",
                            e
                        ))
                    })?;
        ...
        self.db.write(batch).map_err(|e| {
            ...
            TeleologicalStoreError::rocksdb_op("delete_batch", CF_CAUSAL_RELATIONSHIPS, Some(id), e)
        })?;
...
mod tests {
    ...
    fn test_full_state_verification_store_and_get() {
        ...
        // Store
        let stored_id =
            h.rt.block_on(h.store.store_causal_relationship(&rel))
                .expect("Store failed");
        assert_eq!(
            stored_id, expected_id,
            "Store should return the relationship ID"
        );

        // Get and verify
        let retrieved =
            h.rt.block_on(h.store.get_causal_relationship(stored_id))
                .expect("Get failed")
                .expect("Relationship not found");

        assert_eq!(retrieved.id, expected_id, "ID mismatch");
        assert_eq!(
            retrieved.source_fingerprint_id, source_id,
            "Source ID mismatch"
        );
        assert_eq!(
            retrieved.mechanism_type, "direct",
            "Mechanism type mismatch"
        );
        assert!(
            (retrieved.confidence - 0.85).abs() < 0.001,
            "Confidence mismatch"
        );
        assert_eq!(
            retrieved.e5_as_cause.len(),
            CausalRelationship::E5_DIM,
            "E5 cause dim mismatch"
        );
        assert_eq!(
            retrieved.e5_as_effect.len(),
            CausalRelationship::E5_DIM,
            "E5 effect dim mismatch"
        );
        assert_eq!(
            retrieved.e1_semantic.len(),
            CausalRelationship::E1_DIM,
            "E1 dim mismatch"
        );

        // Count
        let count =
            h.rt.block_on(h.store.count_causal_relationships())
                .expect("Count failed");
    ...
    fn test_full_state_verification_secondary_index() {
        ...
        let by_source =
            h.rt.block_on(h.store.get_causal_relationships_by_source(source_id))
    ...
    fn test_full_state_verification_search() {
        ...
        let results =
            h.rt.block_on(
                h.store
                    .search_causal_relationships(&query_embedding, 5, None),
            )
        ...
        let direct_results =
            h.rt.block_on(
                h.store
                    .search_causal_relationships(&query_embedding, 5, Some("direct")),
            )
            .expect("Search failed");
    ...
    fn test_full_state_verification_delete() {
        ...
        // Delete
        let deleted =
            h.rt.block_on(h.store.delete_causal_relationship(rel_id))
        ...
        let retrieved =
            h.rt.block_on(h.store.get_causal_relationship(rel_id))
                .expect("Get failed");
    ...
    fn test_edge_case_empty_database() {
        ...
        // Get non-existent
        let result =
            h.rt.block_on(h.store.get_causal_relationship(Uuid::new_v4()))
                .expect("Get failed");
        assert!(result.is_none(), "Should return None for non-existent ID");

        // Get by source non-existent
        let by_source =
            h.rt.block_on(h.store.get_causal_relationships_by_source(Uuid::new_v4()))
                .expect("Get by source failed");
        assert!(
            by_source.is_empty(),
            "Should return empty vec for non-existent source"
        );

        // Search empty
        let query = vec![0.1f32; 1024];
        let results =
            h.rt.block_on(h.store.search_causal_relationships(&query, 10, None))
                .expect("Search failed");
        assert!(
            results.is_empty(),
            "Search should return empty on empty database"
        );
