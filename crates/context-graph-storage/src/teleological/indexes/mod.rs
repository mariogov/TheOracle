...
//!
//! // Get all HNSW configs (16 total, post-E14 BGE-M3 Dense)
...
//! use uuid::Uuid;
//!
//! // Create registry with all 16 HNSW indexes
...
// New MCP-server callers MUST use these so bad input returns a structured
// error instead of crashing the process.
pub use metrics::{
    try_compute_distance, try_cosine_similarity, try_distance_to_similarity,
    try_recommended_metric, MetricError,
};
...
mod tests {
    ...
    fn test_module_exports_dimension_constants() {
        ...
        assert_eq!(TOPIC_PROFILE_DIM, 14);
    ...
    fn test_all_acceptance_criteria() {
        ...
        // AC1: EmbedderIndex enum: 19 variants (post-E14 BGE-M3 Dense)
        println!("AC1: EmbedderIndex has 19 variants");
        let hnsw = EmbedderIndex::all_hnsw();
        assert_eq!(hnsw.len(), 16); // 19 - 3 (E6, E12, E13)
        println!("    - 16 HNSW variants verified");
        ...
        // AC6: all_hnsw_configs returns 16 entries (post-E14 BGE-M3 Dense)
        println!();
        println!("AC6: all_hnsw_configs returns HashMap with 16 entries");
        let configs = all_hnsw_configs();
        assert_eq!(configs.len(), 16);
        println!("    PASS");
    ...
    fn test_task_core_007_acceptance_criteria() {
        ...
        // AC2: HnswEmbedderIndex for all 16 HNSW embedders
        println!();
        println!("AC2: HnswEmbedderIndex works for all 16 HNSW embedders");
        ...
        // AC3: EmbedderIndexRegistry manages 16 indexes (post-E14 BGE-M3 Dense)
        println!();
        println!("AC3: EmbedderIndexRegistry manages 16 indexes");
        let registry = EmbedderIndexRegistry::new();
        assert_eq!(registry.len(), 16);
        println!("    PASS");
    ...
    fn test_full_state_verification() {
        ...
        let expected: Vec<(EmbedderIndex, usize)> = vec![
            ...
            (EmbedderIndex::E14BgeM3Dense, 1024),
        ];
