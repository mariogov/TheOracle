...
pub fn get_column_family_descriptors(block_cache: &Cache) -> Vec<ColumnFamilyDescriptor> {
    vec![
        ...
        ColumnFamilyDescriptor::new(
            ...
            embedder_edges_options(block_cache),
        ),
        ColumnFamilyDescriptor::new(cf_names::TYPED_EDGES, typed_edges_options(block_cache)),
        ColumnFamilyDescriptor::new(
            cf_names::TYPED_EDGES_BY_TYPE,
            typed_edges_by_type_options(block_cache),
        ),
    ]
}

/// Get ALL column family descriptors: base + teleological + code + causal + ME-JEPA.
///
...
/// - 39 ME-JEPA CFs used by inference, evaluation, healing, hygiene, and flywheel tasks
///
/// # Example
/// ```ignore
/// use rocksdb::Cache;
...
pub fn get_all_column_family_descriptors(block_cache: &Cache) -> Vec<ColumnFamilyDescriptor> {
    ...
    descriptors.extend(crate::dynamicjepa::column_families::dj_cf_descriptors(
        block_cache,
    ));
    descriptors.extend(mejepa_cf_descriptors());
    descriptors
}

/// Total number of column families in a fully configured Context Graph database.
/// Base (11: 8 original + 3 graph linking) + Teleological (41) + Quantized Embedder (14)
/// + Code (5) + Causal (2) + DynamicJEPA (29) + ME-JEPA (44) = 146
///   Teleological 41 = 5 original + 1 content + 1 source_metadata + 1 file_index + 1 topic_portfolio
...
///   + 1 hnsw_graphs + 1 training_records + 1 learning_events + 1 constellations + 1 constellation_by_selector
///   + 1 contrastive_pairs + 1 contrastive_by_kind + 1 contrastive_by_anchor
///   + 1 typed_edge_records + 1 typed_edge_validations + 11 learner-state UTL column families
///   + 1 learner_training_datasets matrix-shard CF.
pub const TOTAL_COLUMN_FAMILIES: usize = cf_names::ALL.len()
    + crate::teleological::column_families::TELEOLOGICAL_CF_COUNT
    + crate::teleological::column_families::QUANTIZED_EMBEDDER_CF_COUNT
    + crate::teleological::column_families::CODE_CF_COUNT
    + crate::teleological::column_families::CAUSAL_CF_COUNT
    + crate::dynamicjepa::column_families::DJ_CF_COUNT
    + context_graph_mejepa_cf::MEJEPA_HYGIENE_REFERENCED_CF_COUNT;

fn mejepa_cf_descriptors() -> Vec<ColumnFamilyDescriptor> {
...
#[cfg(test)]
mod tests {
    use crate::QUANTIZED_EMBEDDER_CF_COUNT;
    ...
    fn test_get_descriptors_returns_11() {
        ...
        assert_eq!(
            descriptors.len(),
            11,
            "Must return exactly 11 descriptors (8 base + 3 graph linking)"
        );
    }
    ...
    fn test_total_column_families_constant() {
        // #645-audit: prior assertion expected 146 CFs (102 base + 44 ME-JEPA)
        // but the count has grown to 234 as ME-JEPA / DynamicJEPA / labels /
        // provenance / heal CFs were added. The test had drifted into a
        // stale-arithmetic failure mode. The new assertion enforces a
        // monotonic floor (>= 146) so additive CF growth doesn't break CI,
        // but any accidental CF deletion or array reordering still fires.
        // If a future schema rev intentionally removes CFs, bump the floor
        // downward at that commit and add a comment.
        assert!(
            TOTAL_COLUMN_FAMILIES >= 146,
            "TOTAL_COLUMN_FAMILIES regressed below the 146 floor (current = {}); did a CF get removed by accident?",
    ...
    fn test_all_cf_descriptors_includes_quantized_embedder_cfs() {
        ...
        // Verify all quantized embedder CFs are present.
        for i in 0..QUANTIZED_EMBEDDER_CF_COUNT {
    ...
    fn test_all_cf_descriptors_includes_teleological_cfs() {
        ...
        // Verify representative teleological CFs are present.
        ...
        assert!(
            names.contains(&"custom_weight_profiles"),
            "Missing CF: custom_weight_profiles"
        );
        assert!(
            names.contains(&"learning_events"),
            "Missing CF: learning_events"
        );
    }
}
