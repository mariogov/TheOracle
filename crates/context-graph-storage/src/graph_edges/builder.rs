...
pub struct BackgroundGraphBuilder {
    ...
    /// Wakes the worker immediately during shutdown instead of waiting for the
    /// batch interval sleep to expire.
...
impl BackgroundGraphBuilder {
    ...
    async fn build_graphs_for_fingerprints(
        ...
        let edge_config = EdgeBuilderConfig::default()
            .with_min_weighted_agreement(self.config.min_weighted_agreement);
        ...
        for (embedder_id, graph) in &build_result.graphs {
            ...
            for source_id in graph.nodes() {
                ...
                if !neighbors.is_empty() {
                    self.edge_repository.store_embedder_edges(
                        *embedder_id,
                        source_id,
                        neighbors,
                    )?;
    ...
    pub async fn rebuild_all(&self) -> GraphEdgeStorageResult<RebuildResult> {
        ...
        info!(
            "rebuild_all: scanned {} fingerprints from store",
            total_count
        );
    ...
    async fn process_batch_internal(
        ...
        info!(
            ...
            "process_batch_internal: drained {} fingerprint IDs from queue", processed_count
        );
