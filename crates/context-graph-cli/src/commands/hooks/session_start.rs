...
fn build_previous_session_context(
    ...
    for mem in cached.iter().take(5) {
        let truncated = if mem.content.len() > 200 {
            format!(
                "{}...",
                &mem.content[..mem.content.floor_char_boundary(200)]
            )
        } else {
            mem.content.clone()
        };
...
fn compute_drift_metrics(current: &SessionSnapshot, previous: &SessionSnapshot) -> DriftMetrics {
    ...
    let coherence_phase_drift =
        ((integration_diff + reflection_diff + differentiation_diff) / 3.0) as f64;
...
mod tests {
    ...
    async fn tc_hooks_013_01_drift_metrics_computed_when_linking() {
        ...
        {
            let mut prev_snapshot = SessionSnapshot::new(prev_id);
            prev_snapshot.topic_profile = [0.5; 14];
    ...
    async fn tc_hooks_017_01_complete_identity_state_restoration() {
        ...
        let prev_purpose = [
            0.3_f32, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 0.1, 0.2, 0.35, 0.45, 0.55, 0.65,
        ];
        ...
        let prev_differentiation = 0.59;
        {
            ...
            prev_snapshot.append_to_trajectory([0.1; 14]);
            prev_snapshot.append_to_trajectory([0.2; 14]);
            store_in_cache(&prev_snapshot);
        }
    ...
    async fn tc_hooks_017_02_stability_healthy_after_restoration() {
        ...
        {
            let mut prev_snapshot = SessionSnapshot::new(prev_id);
            prev_snapshot.topic_profile = [0.5; 14]; // Non-zero vector
    ...
    async fn tc_hooks_017_03_stability_delta_near_zero() {
        ...
        {
            let mut prev_snapshot = SessionSnapshot::new(prev_id);
            prev_snapshot.topic_profile = [0.6; 14];
