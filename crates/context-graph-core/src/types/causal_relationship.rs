//! CausalRelationship type for storing causal descriptions with source provenance.
//!
//! This module defines the [`CausalRelationship`] struct that stores:
//! - causal explanations retained from the legacy graph subsystem
...
// ============================================================================
// RETIRED GENERATOR PROVENANCE COMPATIBILITY
// ============================================================================

/// Retired compatibility metadata for pre-ME-JEPA generated content.
///
/// The local LLM causal/graph agents were deleted after the ME-JEPA pivot. This
/// type remains only so older serialized causal records can still deserialize;
/// active post-pivot code must not populate it or use it as a planning signal.
///
/// # Example
///
/// ```ignore
/// let provenance = LLMProvenance {
///     model_name: "external-validator".to_string(),
///     model_version: "validator-version".to_string(),
...
pub struct LLMProvenance {
    /// Model or validator display name.
    ...
    /// Legacy generator time in milliseconds.
    #[serde(default)]
...
/// A causal relationship retained by the legacy graph subsystem.
///
/// Contains a causal explanation with E5 dual embeddings for
...
pub struct CausalRelationship {
    ...
    // explanation text. This prevents clustering of explanations
    ...
    /// Used to prevent explanation-template clustering by anchoring to unique source.
    #[serde(default)]
    ...
    /// Retired compatibility metadata from the deleted local LLM causal/graph
    /// agents. Active ME-JEPA code must leave this as `None`; the field remains
    /// to preserve old serialized record layout.
    #[serde(default)]
...
impl CausalRelationship {
    ...
    /// * `explanation` - causal explanation text
    ...
    /// rather than the explanation text, preventing clustering of
    ...
    pub fn validate_spans(&self) -> bool {
        ...
        self.source_spans
            .iter()
            .all(|span| span.is_valid_for(source_len) && span.matches_source(&self.source_content))
    }
