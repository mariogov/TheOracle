...
pub enum SourceType {
    ...
    Manual,
    /// Legacy generated causal explanation.
    ///
    /// The pre-pivot generator path is retired. This source type remains only
    /// so older records can still be read and classified.
    CausalExplanation,
    /// Unknown source
    Unknown,
}
    ...
    pub fn display_string(&self) -> String {
        match self.source_type {
            ...
            SourceType::CausalExplanation => {
                ...
                format!(
                    "Source: CausalExplanation[{}] (confidence: {:.2})",
                    mech, conf
                )
            }
...
mod tests {
    ...
    fn test_md_file_chunk_display_with_lines() {
        ...
        assert!(
            display.contains("/docs/readme.md:10-45"),
            "Display should contain path:lines, got: {}",
            display
        );
        assert!(
            display.contains("2/3"),
            "Display should contain chunk info, got: {}",
            display
        );
    }
    ...
    fn test_md_file_chunk_display_without_lines() {
        ...
        assert!(
            !display.contains(":10"),
            "Display without lines should not have line numbers"
        );
        assert!(
            display.contains("/docs/readme.md"),
