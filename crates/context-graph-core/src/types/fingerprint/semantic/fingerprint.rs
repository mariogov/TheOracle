...
//!
//! This module implements the canonical 14-embedding array storage structure.
...
use crate::weights::E11_ENTITY_ENABLED;
...
/// - Dense: Fixed-length f32 vectors (E1, E2-E5, E7-E11, E14)
...
pub enum ValidationError {
    ...
    #[error(
        "Sparse indices ({indices_len}) and values ({values_len}) length mismatch for {embedder}"
    )]
...
/// SemanticFingerprint: Stores all 14 embeddings without fusion.
///
/// # Philosophy
///
...
pub struct SemanticFingerprint {
    ...
    /// E14: BGE-M3 Dense (XLM-RoBERTa-Large) - 1024D multilingual dense embedding.
    ///
    /// `#[serde(default)]` ensures backward compatibility with persisted fingerprints
    /// that pre-date E14. Missing E14 in old records deserializes to an empty `Vec`,
    /// and callers must treat empty slices as "inactive" in fusion/similarity paths.
    #[serde(default)]
    pub e14_bge_m3_dense: Vec<f32>,
}
...
impl SemanticFingerprint {
    ...
    pub fn zeroed() -> Self {
        Self {
            ...
            e14_bge_m3_dense: vec![0.0; E14_DIM],
        }
    }
    ...
    pub fn stub() -> Self {
        Self {
            ...
            e12_late_interaction: Vec::new(),
            e13_splade: SparseVector::empty(),
            e14_bge_m3_dense: vec![0.1; E14_DIM],
        }
    }
    ...
    pub fn get_embedding(&self, idx: usize) -> Option<EmbeddingSlice<'_>> {
        match idx {
            ...
            13 => Some(EmbeddingSlice::Dense(&self.e14_bge_m3_dense)),
            _ => None,
        }
    }
    ...
    /// Embedder enum from teleological::embedder. It never fails since all 14
    /// embedders are addressable.
    ///
    /// # Example
    ///
    /// ```
    ...
    pub fn get(&self, embedder: Embedder) -> EmbeddingRef<'_> {
        match embedder {
            ...
            // E14 BGE-M3 dense is a first-class 1024D field on the fingerprint.
            // Legacy fingerprints (persisted before E14) deserialize with an
            // empty `Vec` via `#[serde(default)]`; callers must treat an empty
            // slice as "inactive" in fusion/similarity paths.
            Embedder::BgeM3Dense => EmbeddingRef::Dense(&self.e14_bge_m3_dense),
        }
    }

    /// Check if all dense embeddings have valid (non-zero) dimensions.
    ///
    /// Returns `true` if all dense embeddings have their expected dimensions.
    ...
    /// - All E5 fields empty (retired/inactive slot)
    ...
    pub fn is_complete(&self) -> bool {
        // E5 check: retired empty slot OR new dual vectors OR legacy single vector.
        ...
        // E11 check: disabled E11 may be empty; legacy/full fingerprints may
        // still carry a correctly-shaped E11 vector that is gated out elsewhere.
        let e11_complete =
            self.e11_entity.len() == E11_DIM || (!E11_ENTITY_ENABLED && self.e11_entity.is_empty());
        ...
        self.e1_semantic.len() == E1_DIM
            ...
            && e10_complete
            && e11_complete
            && self.e14_bge_m3_dense.len() == E14_DIM
    }
    ...
    ///
    /// This method converts all 14 embeddings to dense Vec<f32> format:
    /// - Dense embeddings (E1-E5, E7-E11, E14): Cloned directly
    ...
    /// # Returns
    ///
    /// Array of 14 dense vectors with expected dimensions:
    ...
    pub fn to_cluster_array(&self) -> [Vec<f32>; 14] {
        ...
        [
            ...
            // E14 BGE-M3 dense — first-class 1024D field.
            // Missing/legacy records deserialize to an empty Vec; emit a zeroed
            // 1024D vector so the cluster array shape is stable.
            if self.e14_bge_m3_dense.is_empty() {
                vec![0.0; E14_DIM]
            } else {
                self.e14_bge_m3_dense.clone()
            },
        ]
    }
...
impl PartialEq for SemanticFingerprint {
    fn eq(&self, other: &Self) -> bool {
        self.e1_semantic == other.e1_semantic
            ...
            && self.e14_bge_m3_dense == other.e14_bge_m3_dense
    }
}
