...
fn test_semantic_fingerprint_zeroed() {
    ...
    // E10 uses dual vectors for asymmetric paraphrase/context similarity
    assert!(fp.e10_multimodal_paraphrase.iter().all(|&v| v == 0.0));
...
fn test_semantic_fingerprint_dimensions() {
    ...
    assert!(
        ...
        "Legacy e5_causal should be empty in new format"
    );
    ...
    assert!(
        ...
        "Legacy e8_graph should be empty in new format"
    );
    ...
    assert_eq!(fp.e14_bge_m3_dense.len(), E14_DIM);

    let expected_total = E1_DIM
        + E2_DIM
        + E3_DIM
        + E4_DIM
        + E5_DIM
    ...
    assert_eq!(TOTAL_DENSE_DIMS, 9472);
}
...
fn test_semantic_fingerprint_get_embedding() {
    ...
    assert!(fp.get_embedding(NUM_EMBEDDERS).is_none());
...
fn test_embedding_name() {
    ...
    assert_eq!(
        SemanticFingerprint::embedding_name(13),
        Some("E14_BgeM3Dense")
    );
    assert_eq!(SemanticFingerprint::embedding_name(14), None);
}
...
fn test_embedding_dim() {
    ...
    assert_eq!(SemanticFingerprint::embedding_dim(13), Some(E14_DIM));
    assert_eq!(SemanticFingerprint::embedding_dim(14), None);
}
