...
//! use context_graph_core::embeddings::config::get_dimension;
...
//!
//! // Insert a memory with all 14 embeddings
//! let memory_id = Uuid::new_v4();
//! let embeddings: [Vec<f32>; 14] = std::array::from_fn(|i| {
//!     let embedder = Embedder::from_index(i).unwrap();
//!     vec![0.01 * (i as f32 + 1.0); get_dimension(embedder)]
//! });
//!
//! // Insert into manager
...
use crate::weights::{E11_ENTITY_ENABLED, E5_CAUSAL_ENABLED};

use super::birch::{birch_defaults, BIRCHParams, BIRCHTree};
use super::cluster::Cluster;
use super::error::ClusterError;
use super::fingerprint_matrix::{
    build_fingerprint_matrix, FingerprintMatrixConfig, SimilarityStats,
};
...
/// Maximum weighted agreement per constitution.
///
/// Formula: 8 semantic embedders (1.0) + 2 relational (0.5) + 1 structural (0.5) + E14 BGE-M3 (1.0) = 9.5.
/// Updated post-E14 from 8.5 to 9.5; E14 is treated as a primary semantic signal.
pub const MAX_WEIGHTED_AGREEMENT: f32 = 9.5;
...
/// Whether a space participates in live clustering/topic synthesis.
///
/// Retired/disabled embedders remain addressable in persisted fingerprints for
/// backward compatibility, but they must not influence new clusters or topics.
#[inline]
fn cluster_embedder_enabled(embedder: Embedder) -> bool {
    match embedder {
        Embedder::Causal => E5_CAUSAL_ENABLED,
        Embedder::Entity => E11_ENTITY_ENABLED,
        _ => true,
    }
}
...
impl MultiSpaceClusterManager {
    ...
    fn init_spaces(params: &ManagerParams) -> Result<[PerSpaceState; 14], ClusterError> {
    ...
    /// let memory_id = Uuid::new_v4();
    ///
    /// // Create all 14 embeddings with dimensions from the canonical embedder config
    /// let embeddings: [Vec<f32>; 14] = std::array::from_fn(|i| {
    ///     use context_graph_core::embeddings::config::get_dimension;
    ///     use context_graph_core::teleological::Embedder;
    ///
    ...
    pub fn insert(
        &mut self,
        memory_id: Uuid,
        embeddings: &[Vec<f32>; 14],
    ) -> Result<InsertResult, ClusterError> {
        let mut cluster_indices: [i32; 14] = [-1; 14];
        let mut needs_recluster = false;

        // Insert into each space's BIRCH tree
        for (i, embedder) in Embedder::all().enumerate() {
            let embedding = &embeddings[i];
            if !cluster_embedder_enabled(embedder) {
                tracing::debug!(
                    embedder = ?embedder,
                    vector_dim = embedding.len(),
                    "Skipping disabled embedder during cluster insert"
                );
                continue;
            }
    ...
    pub fn recluster(&mut self) -> Result<ReclusterResult, ClusterError> {
        ...
        let mut per_space_clusters: [usize; 14] = [0; 14];

        for (i, embedder) in Embedder::all().enumerate() {
            if !cluster_embedder_enabled(embedder) {
                self.spaces[i].updates_since_recluster = 0;
                continue;
            }
            let state = &mut self.spaces[i];
            ...
            let memberships = clusterer.fit(&state.embeddings, &state.memory_ids, embedder)?;
            ...
            for (j, membership) in memberships.iter().enumerate() {
                state
                    .memberships
                    .insert(membership.memory_id, membership.clone());
    ...
    pub fn recluster_fingerprint_matrix(
        ...
        tracing::debug!(silhouette = silhouette, "FDMC clustering quality");
    ...
    fn synthesize_topics(&mut self) -> Result<(), ClusterError> {
        ...
        let space_cluster_counts: HashMap<Embedder, usize> = Embedder::all()
            .enumerate()
            .filter(|(_, embedder)| cluster_embedder_enabled(*embedder))
            .map(|(i, embedder)| {
        ...
        let discriminating_spaces: HashSet<Embedder> = space_cluster_counts
            .iter()
        ...
        let space_id_to_idx: Vec<HashMap<&Uuid, usize>> = self
            .spaces
            .iter()
            .map(|state| {
                state
        ...
        let split_spaces: Vec<Embedder> = {
            let mut spaces: Vec<(Embedder, usize)> = discriminating_spaces
                .iter()
                .map(|&e| (e, *space_cluster_counts.get(&e).unwrap_or(&0)))
                .collect();
            spaces.sort_by_key(|(_, count)| std::cmp::Reverse(*count)); // most clusters first
    ...
    fn compute_topic_profile(&self, cluster_indices: &[i32; 14]) -> TopicProfile {
        let mut strengths = [0.0f32; 14];

        for (i, &cluster_idx) in cluster_indices.iter().enumerate() {
            let Some(embedder) = Embedder::from_index(i) else {
                continue;
            };
            if !cluster_embedder_enabled(embedder) {
                continue;
    ...
    pub fn get_memberships(&self, memory_id: Uuid) -> [Option<ClusterMembership>; 14] {
        let mut result: [Option<ClusterMembership>; 14] = Default::default();
    ...
    pub fn updates_status(&self) -> [UpdateStatus; 14] {
        let mut result: [UpdateStatus; 14] = [UpdateStatus::default(); 14];
...
pub struct ReclusterResult {
    ...
    pub per_space_clusters: [usize; 14],
...
pub struct FdmcResult {
    ...
    pub embedder_contributions: [f32; 14],
...
impl FdmcResult {
    ...
    pub fn empty() -> Self {
        Self {
            ...
            embedder_contributions: [0.0f32; 14],
...
mod tests {
    ...
    fn create_test_embeddings(base_value: f32) -> [Vec<f32>; 14] {
        std::array::from_fn(|i| {
            let embedder = Embedder::from_index(i).unwrap();
    ...
    fn create_clustered_embeddings(cluster_id: usize) -> [Vec<f32>; 14] {
        std::array::from_fn(|i| {
            let embedder = Embedder::from_index(i).unwrap();
            let dim = get_dimension(embedder);
    ...
    fn test_manager_params_defaults() {
        ...
        assert_eq!(
            params.recluster_threshold, DEFAULT_RECLUSTER_THRESHOLD,
    ...
    fn test_insert_single_memory() {
        ...
        // Active cluster indices should be valid (>= 0) after BIRCH insert.
        // Retired/disabled embedders remain at -1 and must not influence topics.
        for (i, &idx) in insert_result.cluster_indices.iter().enumerate() {
            let embedder = Embedder::from_index(i).unwrap();
            if cluster_embedder_enabled(embedder) {
                assert!(
                    idx >= 0,
                    "Cluster index for active space {} should be >= 0, got {}",
                    i,
                    idx
                );
            } else {
                assert_eq!(
                    idx, -1,
                    "Cluster index for disabled space {} should stay -1, got {}",
                    i, idx
                );
            }
        }
    ...
    fn test_insert_dimension_mismatch() {
        ...
        println!("[PASS] test_insert_dimension_mismatch - error: {}", err);
    }
    ...
    fn test_recluster_insufficient_data() {
        ...
        assert_eq!(
            ...
            "No clusters with insufficient data"
        );
    ...
    fn test_needs_recluster_false_initially() {
        let manager = MultiSpaceClusterManager::with_defaults().unwrap();
        assert!(
            !manager.needs_recluster(),
            "Should not need recluster initially"
        );
    ...
    fn test_needs_recluster_after_threshold() {
        ...
        assert!(
            manager.needs_recluster(),
            "Should need recluster after threshold"
        );
    ...
    fn test_needs_recluster_reset_after_recluster() {
        ...
        assert!(
            !manager.needs_recluster(),
            "Should not need recluster after reclustering"
        );
    ...
    fn test_updates_status() {
        ...
        // Active spaces should be 1; disabled spaces stay at 0 because they are
        // intentionally gated out of clustering.
        let status = manager.updates_status();
        for update in &status {
            let expected = if cluster_embedder_enabled(update.embedder) {
                1
            } else {
                0
            };
            assert_eq!(
                update.updates_since_recluster, expected,
                "{:?} should have {} update(s)",
                update.embedder, expected
            );
        }
    ...
    fn test_insert_result_needs_recluster() {
        ...
        assert!(
            !result1.needs_recluster,
            "First insert should not trigger recluster"
        );
        ...
        assert!(
            result2.needs_recluster,
            "Second insert should trigger recluster"
        );
    ...
    fn test_edge_case_single_memory_all_spaces() {
        ...
        // Single memory should be assigned to clusters in active spaces only.
        for (i, &idx) in result.cluster_indices.iter().enumerate() {
            let embedder = Embedder::from_index(i).unwrap();
            if cluster_embedder_enabled(embedder) {
                assert!(idx >= 0, "Active space {} should have cluster >= 0", i);
            } else {
                assert_eq!(idx, -1, "Disabled space {} should stay unclustered", i);
            }
        }
    ...
    fn test_constants_match_constitution() {
        assert_eq!(
            MAX_WEIGHTED_AGREEMENT, 9.5,
            "MAX_WEIGHTED_AGREEMENT should be 9.5 per constitution (post-E14)"
        );
        assert_eq!(
            TOPIC_THRESHOLD, 2.5,
    ...
    fn test_import_replaces_existing_topics() {
        ...
        assert_eq!(
            manager.topic_count(),
            0,
            "Import should replace existing topics"
        );
    ...
    fn create_domain_fingerprint(domain: &str, variation: f32) -> SemanticFingerprint {
        ...
        let e5_vec = generate_domain_embedding(
            get_dimension(Embedder::Causal),
            domain,
            variation,
            seed + 400,
        );
        SemanticFingerprint {
            e1_semantic: generate_domain_embedding(
                get_dimension(Embedder::Semantic),
                domain,
                variation,
                seed,
            ),
            e2_temporal_recent: generate_domain_embedding(
                get_dimension(Embedder::TemporalRecent),
                "all",
                0.5,
                seed + 100,
            ),
            e3_temporal_periodic: generate_domain_embedding(
                get_dimension(Embedder::TemporalPeriodic),
                "all",
                0.5,
                seed + 200,
            ),
            e4_temporal_positional: generate_domain_embedding(
                get_dimension(Embedder::TemporalPositional),
                "all",
                0.5,
                seed + 300,
            ),
            ...
            e7_code: generate_domain_embedding(
                get_dimension(Embedder::Code),
                domain,
                variation,
                seed + 500,
            ),
            e8_graph_as_source: generate_domain_embedding(
                get_dimension(Embedder::Graph),
                "all",
                0.5,
                seed + 600,
            ),
            e8_graph_as_target: generate_domain_embedding(
                get_dimension(Embedder::Graph),
                "all",
                0.5,
                seed + 601,
            ...
            e10_multimodal_paraphrase: generate_domain_embedding(
                get_dimension(Embedder::Contextual),
                domain,
                variation,
                seed + 800,
            ),
            e10_multimodal_as_context: generate_domain_embedding(
                get_dimension(Embedder::Contextual),
                domain,
                variation,
                seed + 801,
            ),
            e11_entity: generate_domain_embedding(
                get_dimension(Embedder::Entity),
                domain,
                variation,
                seed + 900,
            ),
            e12_late_interaction: vec![
                generate_domain_embedding(
                    128,
                    domain,
                    variation,
                    seed + 1000
            ...
            e14_bge_m3_dense: generate_domain_embedding(
                get_dimension(Embedder::BgeM3Dense),
                domain,
                variation,
                seed + 1100,
            ),
    ...
    fn test_fdmc_insufficient_fingerprints() {
        ...
        println!(
            "[PASS] test_fdmc_insufficient_fingerprints - correctly handles < min_cluster_size"
        );
    }
    ...
    fn test_fdmc_single_cluster() {
        ...
        let cluster_ids: Vec<i32> = fdmc_result
            .memberships
            .iter()
            .map(|m| m.cluster_id)
            .collect();
    ...
    fn create_15domain_fingerprint(domain_idx: usize, variation: f32) -> SemanticFingerprint {
        ...
        fn generate_15domain_embedding(
            dim: usize,
            domain_idx: usize,
            variation: f32,
            seed: u64,
        ...
        let e5_vec = generate_15domain_embedding(
            get_dimension(Embedder::Causal),
            domain_idx,
            variation,
            seed + 400,
        ...
        SemanticFingerprint {
            e1_semantic: generate_15domain_embedding(
                get_dimension(Embedder::Semantic),
                domain_idx,
                variation,
                seed,
            ),
            e2_temporal_recent: generate_15domain_embedding(
                get_dimension(Embedder::TemporalRecent),
                7,
                0.5,
                seed + 100,
            ),
            e3_temporal_periodic: generate_15domain_embedding(
                get_dimension(Embedder::TemporalPeriodic),
                7,
                0.5,
                seed + 200,
            ),
            e4_temporal_positional: generate_15domain_embedding(
                get_dimension(Embedder::TemporalPositional),
                7,
                0.5,
                seed + 300,
            ...
            e5_causal: Vec::new(), // Using new dual format
            e6_sparse: SparseVector::empty(),
            e7_code: generate_15domain_embedding(
                get_dimension(Embedder::Code),
                domain_idx,
                variation,
                seed + 500,
            ),
            e8_graph_as_source: generate_15domain_embedding(
                get_dimension(Embedder::Graph),
                7,
                0.5,
                seed + 600,
            ),
            e8_graph_as_target: generate_15domain_embedding(
                get_dimension(Embedder::Graph),
                7,
                0.5,
                seed + 601,
            ...
            e10_multimodal_paraphrase: generate_15domain_embedding(
                get_dimension(Embedder::Contextual),
                domain_idx,
                variation,
                seed + 800,
            ),
            e10_multimodal_as_context: generate_15domain_embedding(
                get_dimension(Embedder::Contextual),
                domain_idx,
                variation,
                seed + 801,
            ),
            e11_entity: generate_15domain_embedding(
                get_dimension(Embedder::Entity),
                domain_idx,
                variation,
                seed + 900,
            ...
            e13_splade: SparseVector::empty(),
            e14_bge_m3_dense: generate_15domain_embedding(
                get_dimension(Embedder::BgeM3Dense),
                domain_idx,
                variation,
                seed + 1100,
    ...
    fn test_fdmc_15_topics_discovery() {
        ...
        for (domain_idx, domain_name) in DOMAINS_15.iter().enumerate() {
            ...
            println!(
                "Created 3 fingerprints for domain {}: {}",
                domain_idx, domain_name
            );
        }
        ...
        assert_eq!(
            fingerprints.len(),
            45,
            "Should have 45 fingerprints (15 domains × 3)"
        );
        ...
        for (domain_idx, domain_name) in DOMAINS_15.iter().enumerate() {
            ...
            let coherent = domain_labels
                .iter()
                .all(|&l| l == domain_labels[0] && l >= 0);
            println!(
                "  {}: {:?} {}",
                domain_name,
                domain_labels,
    ...
    fn test_fdmc_15_topics_cluster_purity() {
        ...
        for &cluster_id in &unique_clusters {
            ...
            let cluster_members: Vec<usize> = labels
                .iter()
                .enumerate()
            ...
            println!(
                "  Cluster {}: {} members, dominant={} ({}), purity={:.2}%",
                cluster_id,
                cluster_members.len(),
                DOMAINS_15[dominant_domain],
                max_count,
                purity * 100.0
            );
        }

        let avg_purity = if cluster_count > 0 {
            total_purity / cluster_count as f32
        } else {
            0.0
        };
    ...
    fn test_fdmc_15_topics_with_5_per_domain() {
        ...
        println!(
            "\n[PASS] test_fdmc_15_topics_with_5_per_domain - {} topics",
            result.topics_discovered
        );
    }
    ...
    fn test_fdmc_incremental_domain_addition() {
        ...
        for domain_idx in 0..5 {
            for i in 0..3 {
                fingerprints.push((
                    Uuid::new_v4(),
                    create_15domain_fingerprint(domain_idx, i as f32 * 0.1),
                ));
            }
        }
        ...
        println!("\n=== INCREMENTAL TEST ===");
        println!(
            "Phase 1 (5 domains, 15 memories): {} topics",
            result1.topics_discovered
        );
        ...
        println!(
            "Phase 2 (10 domains, 30 memories): {} topics",
            result2.topics_discovered
        );
        ...
        println!(
            "Phase 3 (15 domains, 45 memories): {} topics",
            result3.topics_discovered
        );
    ...
    fn test_fdmc_noise_injection() {
        ...
        println!(
            "Total memories: {} (30 regular + 5 noise)",
            result.memory_count()
        );
        println!(
            "Topics: {}, Clusters: {}",
            result.topics_discovered, result.total_clusters
        );
        println!("Noise points: {}", result.noise_count());
    ...
    fn test_fdmc_similarity_matrix_structure() {
        ...
        println!(
            "Intra-domain avg similarity: {:.4} ({} pairs)",
            intra_domain_avg, intra_count
        );
        println!(
            "Inter-domain avg similarity: {:.4} ({} pairs)",
            inter_domain_avg, inter_count
        );
        println!(
