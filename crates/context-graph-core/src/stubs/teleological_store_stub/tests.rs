...
async fn test_search_semantic() {
    ...
    assert!(
        !results.is_empty(),
        "Search after storing 5 items should return results"
    );
...
async fn test_batch_store_and_retrieve() {
    // V-010 fix (#505): previous version only checked `.is_some()` on each
    // retrieved entry. A broken retrieve_batch returning 10 unrelated
    // fingerprints would have passed. Now we zip ids with retrieved
    // fingerprints and assert id-equality per pair.
    let store = InMemoryTeleologicalStore::new();
    ...
    for (i, (expected_id, fp)) in ids.iter().zip(retrieved.iter()).enumerate() {
        let fp = fp
            .as_ref()
            .unwrap_or_else(|| panic!("retrieved[{i}] missing (id={expected_id})"));
        assert_eq!(
            fp.id, *expected_id,
            "retrieved[{i}] id mismatch: expected {expected_id}, got {}",
            fp.id
        );
    }
}
...
async fn test_file_index_operations() {
    ...
    let removed = store
        .unindex_file_fingerprint(file_path, id1)
        .await
        .unwrap();
    ...
    assert_eq!(
        ids_after.len(),
        1,
        "Should have 1 fingerprint after unindex"
    );
    ...
    assert!(
        !removed_again,
        "Should return false for already-removed fingerprint"
    );
...
async fn test_file_index_multiple_files() {
    ...
    store
        .index_file_fingerprint("/file1.md", id1)
        .await
        .unwrap();
    store
        .index_file_fingerprint("/file2.md", id2)
        .await
        .unwrap();
...
async fn test_search_text_returns_not_implemented_error() {
    ...
    assert!(
        result.is_err(),
        "search_text MUST return error, not Ok(empty)"
    );
